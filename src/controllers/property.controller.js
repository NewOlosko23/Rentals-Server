  ...
  return new Promise((resolve, reject)=>{
    const upload = cloudinary.uploader.upload_stream({
      ...
      transformation: [
        ...
        { overlay: process.env.CLOUDINARY_LOGO_PUBLIC_ID, gravity: 'center', opacity: 60 },
...
export const createProperty = asyncHandler(async (req, res) => {
  ...
  if (req.files?.length){
    for (const f of req.files){
      ...
      media.push({ url: r.secure_url, publicId: r.public_id, width: r.width, height: r.height });
    }
  }
  const doc = await Property.create({
    ...
    price: body.price,
    location: { city: body.city, estate: body.estate },
...
export const getProperty = asyncHandler(async (req, res) => {
  const p = await Property.findByIdAndUpdate(req.params.id, { $inc: { 'stats.views': 1 } }, { new: true });
...
export const updateProperty = asyncHandler(async (req, res) => {
  ...
  const p = await Property.findOneAndUpdate({ _id: req.params.id, lister: req.user.id }, updates, { new: true });
  if (!p) return res.status(404).json({ message: 'Not found or not owner' });
  res.json({ property: p });
});

export const archiveProperty = asyncHandler(async (req, res) => {
  const p = await Property.findOneAndUpdate({ _id: req.params.id, lister: req.user.id }, { status: 'archived' }, { new: true });
  if (!p) return res.status(404).json({ message: 'Not found or not owner' });
  res.json({ property: p });
});
